...
function recordWorldPoint(eventModelId, rawBounds, state, matrix, point) {
...
function inspectPrimitive(eventModelId, primitive, matrix, rawBounds, state) {
  ...
  if (elements % 3 !== 0) throw new Error(`${eventModelId}: incomplete triangle data`);
  state.triangles += elements / 3;
  const point = [0, 0, 0];
  for (let index = 0; index < position.getCount(); index += 1) {
    position.getElement(index, point);
    recordWorldPoint(eventModelId, rawBounds, state, matrix, point);
  }
}

function inspectNode(eventModelId, node, visited, rawBounds, state) {
  ...
  for (const primitive of mesh.listPrimitives()) {
    inspectPrimitive(eventModelId, primitive, matrix, rawBounds, state);
  }
}
...
export function inspectEventModel(eventModelId, document) {
  ...
  for (const scene of scenes) {
    for (const child of scene.listChildren()) {
      child.traverse((node) => inspectNode(eventModelId, node, visited, rawBounds, state));
    }
  }
