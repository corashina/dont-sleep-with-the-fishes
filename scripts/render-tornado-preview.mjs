...
const output = resolve(process.argv[2] ?? 'artifacts/tornado-preview');
const expected = new Set(['tornado', 'tornado-motion', 'tornado-boat', 'tornado-occlusion', 'tornado-night', 'tornado-night-close']);
...
const vite = await createServer({ server: { host: '127.0.0.1', port: 0,
  ...
  name: 'tornado-preview-capture', configureServer(server) {
...
try {
  ...
  const profile = await mkdtemp(resolve(tmpdir(), 'tornado-preview-'));
  browser = spawn(executable, ['--headless=new', '--disable-gpu-sandbox', '--no-first-run',
    '--no-default-browser-check', `--user-data-dir=${profile}`,
    `http://127.0.0.1:${address.port}/dont-sleep-with-the-fishes/scripts/tornado-lab/index.html?capture`,
