...
button.onclick = () => {
  ...
  const frame = (now: number) => {
    ...
    if (now - first >= 10000 && previous) samples.push(now - previous);
    previous = now;
    ...
    const report = { ...summarizeFrameTimes(samples), invalid,
      gpu: info ? gl!.getParameter(info.UNMASKED_RENDERER_WEBGL) : 'unavailable',
      browser: navigator.userAgent, width: canvas?.width, height: canvas?.height,
      pixelRatio: devicePixelRatio, targetMet: !invalid && summarizeFrameTimes(samples).p95Ms <= 16.7 };
