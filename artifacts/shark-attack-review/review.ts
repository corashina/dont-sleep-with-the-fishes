...
const run = async () => {
 ...
 const camera = new PerspectiveCamera(80, 960/600, .1, 500); camera.position.set(0,.88,.96); camera.lookAt(0,.88,-5);
 ...
 const presentation = new SharkSwarmPresentation({ eventModels, camera, cameraEffectsRoot:effects, readWorldWaveAmplitudeScale:()=>0, sampleWorldWaveInto:()=>{} } as never);
 ...
 for(const [n,seed] of [42,123456789].entries()) {
  ...
  void presentation.react({resourceDeltas:{health:-50,hull:-30},selectedInstanceId:null,brokenInstanceIds:[]} as never);
  let last=0;
  for(const [i,t] of [.5,.85,1.15,1.45,1.68,1.8].entries()) {
  ...
  await fetch(`/capture/${n}`,{method:'POST',body:png});
 }
 presentation.dispose();eventModels.dispose();renderer.dispose();
 await fetch('/complete',{method:'POST',body:'Rendered attack frames with production shark model.'});
