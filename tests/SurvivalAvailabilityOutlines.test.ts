...
describe('survival availability outlines', () => {
  it.each([
    ...
    const expected = [
      ...
      'boat-supply:radio', 'boat-supply:medicalKit', 'repair-toolbox', 'fishing-rod',
      ...(energy === 3 ? ['persistent-chest'] : []),
      'sleep-pillow',
    ].sort();
    try {
  ...
  it('removes outlines when their resources, equipment or repair targets are unavailable', () => {
    ...
    try {
      ...
      expect(rig.targets()).toEqual(['boat-supply:energyBar', 'fishing-rod', 'sleep-pillow']);
  ...
  it('only outlines actions without an energy cost when energy is empty', () => {
    ...
    try {
      expect(rig.targets()).toEqual([
        ...
        'boat-supply:medicalKit', 'sleep-pillow',
      ]);
    } finally {
      rig.dispose();
    }
