...
describe('Night Trader presentation', () => {
  it.each([
    ['food-consumable', 'food', 1],
    ['bait-consumable', 'bait', 1],
    ['food-equipment', 'food', 3],
    ['bait-equipment', 'bait', 3],
  ] as const)('reads the %s payment context', async (choiceId, resource, quantity) => {
    ...
    const actor = presentation.root.getObjectByName(
      `night-trader-payment-${resource}-token`,
    );

    expect(actor?.userData.quantity).toBe(quantity);
    presentation.settleForVisibilityChange();
    await payment;
    presentation.dispose();
  });

  it('returns the committed item instead of a fixed choice reward', async () => {
    const presentation = new NightTraderPresentation(dependencies());
    presentation.stage();
    const reaction = presentation.react(
      {
        eventId: 'night-trader',
        choiceId: 'food-consumable',
        resultId: 'trader-reward',
      },
      acceptedReward('scubaSet'),
    );

    expect(presentation.root.getObjectByName('night-trader-reward-scubaSet')).toBeDefined();
    expect(presentation.root.getObjectByName('night-trader-reward-ductTape')).toBeUndefined();
    presentation.settleForVisibilityChange();
    await reaction;
    presentation.dispose();
  });
