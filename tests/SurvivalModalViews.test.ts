...
describe('SurvivalModalViews', () => {
  ...
  it.each([
    [{ id: 'rescue', day: 24, savedPickupCount: 7, signalAssisted: true },
      'RESCUE FOUND YOU',
      'A distant crew followed the signs you left across the sea.',
      '',
      'DAY 24 · 7 PICKUPS SAVED'],
    [{ id: 'death', day: 8, savedPickupCount: 4, cause: { kind: 'starvation' } },
      'THE SEA OUTLASTED YOU',
      'Hunger left you too weak to meet another dawn.',
      '',
      'DAY 8 · 4 PICKUPS SAVED'],
    [{ id: 'sinking', day: 12, savedPickupCount: 5, cause: { eventId: 'chest-attack' } },
      'THE BOAT IS GONE',
      'The last damage opened the boat to the sea.',
      'LAST EVENT: CHEST ATTACK',
      'DAY 12 · 5 PICKUPS SAVED'],
    [{ id: 'taken', day: 20, savedPickupCount: 9 },
      'TAKEN IN THE DARK',
      'The light found something that had been waiting for you.',
      '',
      'DAY 20 · 9 PICKUPS SAVED'],
  ] satisfies readonly [
    Exclude<EndingRecord, { id: 'dorothy' }>,
    string,
    string,
    string,
    string,
  ][])('renders $record.id ending copy', (record, title, body, cause, stats) => {
    const view = createView();
    view.showEnding(record);
    const causeElement = view.endingRoot.querySelector<HTMLElement>('[data-ending-cause]')!;

    expect(view.endingRoot.dataset.ending).toBe(record.id);
    expect(view.endingTitle.textContent).toBe(title);
    expect(view.endingRoot.querySelector('[data-ending-body]')?.textContent).toBe(body);
    expect(causeElement.textContent).toBe(cause);
    expect(causeElement.hidden).toBe(cause.length === 0);
    expect(view.endingRoot.querySelector('[data-ending-stats]')?.textContent).toBe(stats);
  });
  ...
  it('fires Restart once per ending display and ignores inert clicks', () => {
    ...
    view.showEnding({ id: 'sinking', day: 4, savedPickupCount: 2, cause: { eventId: null } });
    ...
    view.showEnding({ id: 'rescue', day: 24, savedPickupCount: 2, signalAssisted: false });
