...
describe('item circulation', () => {
  ...
  it('keeps the Handyman exchange stable through a checkpoint', () => {
    const game = new SurvivalSession(saved('anchor'), {
      seed: 3, initialEventId: 'handyman', initial: { food: 6, bait: 6, day: 20 },
    });
    const restored = SurvivalSession.restore(game.exportCheckpoint());
    const response = { kind: 'item' as const, choiceId: 'anchor', instanceId: 'anchor-1' as const };
