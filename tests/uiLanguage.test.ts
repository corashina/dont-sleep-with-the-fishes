...
describe('live gameplay translations', () => {
  ...
  it('updates active action reasons and preserves armed restart confirmation', () => {
    ...
    ui.setPaused(true);
    const restart = find<HTMLButtonElement>('[data-pause-restart]');
    restart.click();
    restart.focus();
    const action = vi.fn();
    ui.onRestart = action;
    ...
    expect(restart.textContent).toBe('POTWIERDŹ NOWY POCZĄTEK');
