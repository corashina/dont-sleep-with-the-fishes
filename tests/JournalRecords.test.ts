...
describe('journal records', () => {
  ...
  it('constructs independent immutable journal records', () => {
    ...
    if (night.kind !== 'event') throw new Error('Expected an event night record.');
    expect(night.event).not.toBe(event);
    expect(Object.isFrozen(night)).toBe(true);
    expect(Object.isFrozen(night.event)).toBe(true);
    const quietNight = createQuietJournalNightRecord();
    expect(quietNight).toEqual({ kind: 'quiet' });
    expect(Object.isFrozen(quietNight)).toBe(true);
    ...
    expect(Object.isFrozen(dawn)).toBe(true);
    ...
    expect(entry.daytime).not.toBe(sinkingShip);
    expect(Object.isFrozen(entry.daytime)).toBe(true);
    expect(entry.nighttime).not.toBe(night);
    expect(Object.isFrozen(entry.nighttime)).toBe(true);
  });
});
