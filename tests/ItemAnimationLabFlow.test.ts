...
describe('ItemAnimationLabFlow', () => {
  ...
  it.each([
    ...
    const uses = ITEM_ANIMATION_LAB_USES[itemType]!;
    const use = uses[0]!;
    rig.flow.enter(rig.session.snapshot());

    await rig.flow.play(instanceId);
    if (uses.length > 1) rig.flow.choose(use.id);
  ...
  it('guards a late keyed audio cue with both generations', async () => {
    const rig = createRig();
    const use = ITEM_ANIMATION_LAB_USES.anchor![0]!;
    const animation = deferred();
    ...
    await rig.flow.play('anchor-1' as ItemInstanceId);
    rig.flow.choose(use.id);
    await vi.waitFor(() => expect(cue).toBeTypeOf('function'));
    rig.restart();
    cue?.(0);
    animation.resolve();
  ...
  it('keeps an activation error primary while all cleanup continues', async () => {
    ...
    await rig.flow.play('swimRing-1' as ItemInstanceId);
