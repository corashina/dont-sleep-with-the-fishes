...
import { FlyingSaucerPresentation,UFO_BEAM_DURATION } from '../src/survival/FlyingSaucerPresentation';
...
describe('flying saucer presentation', () => {
  ...
  it('approaches before the beam opens, then looks up without lifting the camera', async () => {
    ...
    try {
      ...
      await reveal;
      const done = vi.fn();
      const hit = presentation.react(result('ufo-beam-hit'), outcome).then(done);
      const beam = presentation.root.getObjectByName('ufo-beam')!;
      ...
      presentation.update(13, UFO_BEAM_DURATION - 6);
      await hit;
      expect(done).toHaveBeenCalledOnce();
      expect(camera.position.y).toBe(1);
      expect(camera.quaternion.equals(startQuaternion)).toBe(false);
  ...
  it('keeps flying through staging, reveal, and safe departure without moving the camera', async () => {
    ...
    try {
      ...
      expect(presentation.root.getObjectByName('ufo-beam')!.visible).toBe(false);
      presentation.clear();
      expect(camera.position.equals(position)).toBe(true);
      expect(camera.quaternion.equals(quaternion)).toBe(true);
    } finally { presentation.dispose(); }
