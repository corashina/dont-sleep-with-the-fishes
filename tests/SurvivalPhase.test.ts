...
describe('SurvivalPhase orchestration', () => {
  it.each([
    [{ kind: 'resource', id: 'food', quantity: 2 }, '+2 FOOD'],
    [{ kind: 'resource', id: 'bait', quantity: 2 }, '+2 BAIT'],
    [{ kind: 'resource', id: 'repairMaterial', quantity: 2 }, '+2 REPAIR MATERIAL'],
    [{ kind: 'item', id: 'energyBar', quantity: 1 }, 'ENERGY BAR'],
    [{ kind: 'resource', id: 'food', quantity: 1 }, '+1 FOOD'],
  ] satisfies readonly [RewardSummary, string][])(
  ...
  it('finishes a day action without requesting or staging a day event', async () => {
    let current = snapshot();
    const requestDayEvent = vi.fn();
    const stageEvent = vi.fn();
    const phase = SurvivalPhase.forTest({
      session: {
        snapshot: vi.fn(() => current),
        perform: vi.fn(() => {
          current = snapshot({ actedToday: true });
          return accepted({ code: 'ate', cue: 'none' });
        }),
        requestDayEvent,
      },
      world: {
  ...
  it('holds a resolved night event, then advances and renders only while covered', async () => {
    ...
    await flushPromises();
    expect(calls).toEqual(['resolve', 'cue', 'react', 'hold']);
  ...
  it.each([
    ...
    let current = snapshot({
      state,
      pendingEventId: event.id,
      ...(state === 'dayEvent' ? { pendingDriftingLootVariant: 'barrel' } : {}),
    });
    const calls: string[] = [];
    ...
    const phase = SurvivalPhase.forTest({
      ...
      ui: {
        ...
        showFeedback, holdEventOutcome, setBusy: vi.fn(), render: vi.fn(),
  ...
  it('defers item resolution across hidden item and reaction boundaries without night feedback', async () => {
    const listeners = new Map<string, EventListener>();
    const fakeDocument = {
      hidden: false,
      addEventListener: vi.fn((type: string, listener: EventListener) => listeners.set(type, listener)),
