...
describe('SurvivalPhase orchestration', () => {
  ...
  it('holds a resolved night event, then advances and renders only while covered', async () => {
    ...
    await flushPromises();
    expect(calls).toEqual(['resolve', 'cue', 'react', 'outcome', 'hold']);
    ...
    await flushPromises();
    expect(calls).toEqual([
      'resolve', 'cue', 'react', 'outcome', 'hold', 'cover',
      'clear', 'dawn', 'dawn-cue',
    ]);
    expect(calls).not.toContain('uncover');
  ...
  it('integrates Swim Ring bottle recovery through reveal, inventory sync, reaction, and dawn', async () => {
    ...
    expect(calls.indexOf('react:drifting-bottle')).toBeLessThan(calls.indexOf('outcome'));
    expect(calls.indexOf('outcome')).toBeLessThan(calls.indexOf('hold'));
