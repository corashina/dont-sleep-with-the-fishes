...
describe('event presentation audio cue routing', () => {
  ...
  it('routes Check the Back result cues to the survival audio scope', () => {
    ...
    handler!({ eventId: 'check-the-back', cue: 'fish' });
    handler!({ eventId: 'check-the-back', cue: 'anglerfish' });
...
describe('SurvivalPhase orchestration', () => {
  ...
  it('shows one Empty Lifeboat supply before it returns from focus', async () => {
    ...
    expect(rig.showFocusedEvent.mock.calls[0]![0].choices).toMatchObject([
      {
        id: 'search',
        label: 'Search It',
    ...
    rig.ui.onFocusedEventChoice?.({ id: 'search', instanceId: null });
    await flushPromises();
  ...
  it('lets Drifting Cargo recede without a result and resumes the same day', async () => {
    ...
    phase.start();
    await flushPromises();
    ui.onFocusedEventSelect?.('drifting-barrel');
    await flushPromises();
  ...
  it('renders and unlocks an accepted daytime action without text feedback', async () => {
    ...
    phase.handleAction('answerRadio');
    phase.handleAction('answerRadio');
  ...
  it('keeps Handyman Touch still and hides its result popup', async () => {
    let current = snapshot({
      state: 'nightEvent',
      pendingEventId: 'handyman',
      day: 20,
    ...
    const phase = SurvivalPhase.forTest({
      session: {
        ...
        resolveEvent: vi.fn(() => {
          ...
          return accepted({
            code: 'event-resolved',
            message: 'The hand closes around you.',
            deltas: { health: -70, hull: -41 },
            cue: 'repair',
            eventResult: {
              eventId: 'handyman',
              choiceId: 'touch',
  ...
  it('explains unmet resource requirements in contextual choice view models', async () => {
    ...
    await flushPromises();
    expect(setEventSelection.mock.calls[0]![1]).toEqual([]);
    focusUi.onFocusedEventSelect?.('drifting-barrel');
    await flushPromises();
