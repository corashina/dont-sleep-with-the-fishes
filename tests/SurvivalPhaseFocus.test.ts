...
describe('SurvivalPhase focus synchronization', () => {
  ...
  it('rearms rejected fishing input, then makes an accepted catch non-actionable until return', async () => {
    ...
    finishEnter();
    await flushPromises();
    const fishingLayer = mount.querySelector<HTMLElement>('[data-fishing]')!;
    ...
    expect(fishingLive.textContent).toBe('CLICK THE WATER TO CAST');
    expect(biteButton.hidden).toBe(true);
    expect(document.activeElement).toBe(fishingLayer);
    ...
    expect(fishingLive.textContent).toBe('BITE - REEL NOW');
    document.dispatchEvent(new KeyboardEvent('keydown', { key: 'Enter' }));
    await flushPromises();
    ...
    expect(biteButton.hidden).toBe(true);
    expect(document.activeElement).toBe(fishingLayer);
    expect(fishingLive.textContent).toBe('REELING IN');
  ...
  it('makes an automatic miss non-actionable before its deferred animation resolves', async () => {
    ...
    expect(biteButton.hidden).toBe(true);
    expect(document.activeElement).toBe(fishingLayer);
    expect(fishingLive.textContent).toBe('THE LINE WENT SLACK');
    expect(fishingLive.getAttribute('aria-live')).toBe('polite');
    const committed = session.snapshot();
    biteButton.click();
    document.dispatchEvent(new KeyboardEvent('keydown', { key: 'Enter' }));
