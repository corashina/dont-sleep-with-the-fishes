...
describe('DriftingItemView', () => {
  it('owns the exact single root and focus markup', () => {
    ...
    expect(view.root.getAttribute('aria-label')).toBe('Pickup choices');
  ...
  it('keeps generic choice order, copy, energy, and reasons without owner copy', () => {
    const { view } = createView();
    view.show({ eventId: 'drifting-barrel', choices, target: null });
  ...
  it('uses the first available choice and Back only when none are available', () => {
    ...
    view.show({
      eventId: 'drifting-barrel', target: null,
  ...
  it('clones the target and places CSS variables beside it', () => {
    ...
    view.show({ eventId: 'drifting-barrel', choices, target });
  ...
  it('hides and clears choices, target, and selection', () => {
    const { view } = createView();
    const hide = vi.fn();
    view.onHide = hide;
    view.show({ eventId: 'drifting-barrel', choices, target: null });
