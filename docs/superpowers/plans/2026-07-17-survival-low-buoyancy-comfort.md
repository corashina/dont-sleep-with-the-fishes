...
- Retain 10 percent of survival heave and lateral drift.
- Retain 4 percent of survival pitch and roll.
...
- Produces: `SURVIVAL_TRANSLATION_SCALE: 0.1`, `SURVIVAL_ROTATION_SCALE: 0.04`, and `applySurvivalBuoyancyComfortInto(output: BoatPose, source: Readonly<BoatPose>): void`.
...
describe('survival buoyancy comfort', () => {
  it('retains ten percent translation and four percent rotation in caller-owned storage', () => {
    ...
    expect(SURVIVAL_TRANSLATION_SCALE).toBe(0.1);
    expect(SURVIVAL_ROTATION_SCALE).toBe(0.04);
    expect(output).toBe(outputReference);
    expect(output.y).toBeCloseTo(0.2);
    expect(output.pitch).toBeCloseTo(0.02);
    expect(output.roll).toBeCloseTo(-0.01);
    expect(output.driftX).toBeCloseTo(0.04);
    expect(output.driftZ).toBeCloseTo(-0.06);
...
Expected: PASS. The survival motion rig matches the 10 percent translation and 4 percent rotation profile, squall heave exceeds calm heave, and reduced-motion secondary cues remain neutral.
