...
Captain Whiskers is a small, weathered orange-and-white ship cat with a readable
seated silhouette, purposeful asymmetry, and a crooked tarnished collar tag. His
...
The model is an original project-owned GLB authored from simple low-poly forms,
using the unofficial wiki screenshots only as the orange-and-white identity
reference. It is regenerated by `scripts/captain-whiskers-model.mjs`, stays
within the normal item triangle budget, and contains no third-party model data.
The repository attribution ledger records that provenance and the wiki
reference.
