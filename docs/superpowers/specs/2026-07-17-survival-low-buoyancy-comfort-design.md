...
The survival lifeboat will retain 5 percent of the sampled heave and lateral drift. It will retain 2 percent of the sampled pitch and roll. The fixed `SURVIVAL_BOAT_ANCHOR` height remains unchanged, so the hull keeps its authored waterline.
...
- Export the translation scale of `0.05` and rotation scale of `0.02`.
...
Unit tests will verify the exact 5 percent translation and 2 percent rotation values, including writes into the caller-owned output record.
