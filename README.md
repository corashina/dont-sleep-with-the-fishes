...
The event catalog contains 31 live events. The day pool contains Drifting Barrel and
Drifting Chest. Event selection relies on the scene and response prompts.
It does not show event titles.

Starry Night is a positive night event. Binoculars grant one temporary bonus
energy, shown as `4 / 3`; Compass adds one rescue lead.

Constellation Night uses the same positive rewards. Orion appears fully formed
before the event reveal, with its primary stars joined by faint blue-white lines.
...
Event tableaus include drifting flowers, the chest mimic, and an empty lifeboat.
Searching the empty lifeboat costs one energy and always finds one food or one bait.
