...
The ship combines original procedural geometry with locally committed wood maps and flat authored steel materials. Fire and smoke appear on unreachable roofs, machinery, and hull areas. Cosmetic wall leaks feed streams and puddles without changing movement. Broken planks remain visual only. Each enclosed room has a pulsing caged alarm lamp. The CC0 klaxon starts with the active run and follows pause and exit lifecycle.
...
`src/world/ShipDangerEffects` owns fixed hazards, room alarms, fire, smoke, leaks, puddles, and visual-only broken planks.
