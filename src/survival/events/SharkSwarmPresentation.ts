...
const BODY_PRESENTATION_SCALE = 1.55;
const BODY_WATERLINE_FRACTION = 0.82;
...
export class SharkSwarmPresentation implements DedicatedEventPresentation {
  ...
  constructor(private readonly environment: DedicatedEventEnvironment) {
    ...
    for (let index = 0; index < SWARM_SHARK_COUNT; index += 1) {
      ...
      const swimClip = root.animations.find(({ name }) => name === 'Armature|Swim')
        ?? root.animations[0];
      const mixer = swimClip === undefined ? null : new AnimationMixer(root);
      ...
      root.userData.visibleBodyFraction = 1 - BODY_WATERLINE_FRACTION;
      this.sharks.push({
        root,
        modelInstance,
        mixer,
    ...
    for (let index = 0; index < CATCH_COUNT; index += 1) {
      const root = this.sharks[index]!.root.clone(true);
      ...
      root.userData.catchModelId = 'shark';
  ...
  private applySample(time: number): void {
    ...
    for (let index = 0; index < this.sharks.length; index += 1) {
      ...
      const presentationScale = shark.pose.scale * BODY_PRESENTATION_SCALE;
      ...
      shark.root.visible = index < this.sample.bodyVisibleCount && !caught;
    }
  ...
  private prewarmCoveredFrame(): void {
    const firstShark = this.sharks[0];
    if (firstShark === undefined) return;
    firstShark.root.visible = true;
    firstShark.root.scale.setScalar(COVERED_PREWARM_SCALE);
  }
