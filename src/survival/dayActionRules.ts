...
export function dayActionUnavailableReason(
  ...
  switch (action) {
    ...
    case 'sendMessage':
      if (state.rescueMessageSent) return 'You already sent the rescue message.';
      if (!hasUsable(state.inventory, 'bottledPaper')) return 'No bottled paper remains.';
      return state.energy < SURVIVAL_BALANCE.actions.bottledPaperEnergy
        ? 'Sending the message requires one energy.'
        : null;
    case 'useEnergyBar':
