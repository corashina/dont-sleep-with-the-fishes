...
import { NET_ATTACK_CONTACT, NET_ATTACK_CONTACT_PITCH, NET_ATTACK_GRIP, NET_ATTACK_ROLL } from './netAttackChoreography';
...
export class NetAttackPose {
  ...
  private readonly contactRotation = new Quaternion().setFromEuler(
    new Euler(NET_ATTACK_CONTACT_PITCH, 0, NET_ATTACK_ROLL, 'YXZ'),
  );
  ...
  apply(root: Object3D, sample: Readonly<EventItemUseSample>, cameraMatrix: Matrix4, target: Object3D | null): void {
    ...
    if (target !== null) {
      ...
      ).multiply(this.scale).applyQuaternion(this.contactRotation).applyQuaternion(this.aimRotation);
      // Keep the handle on the player side while the head sweeps across the target.
      this.contactGrip.copy(this.target).sub(this.offset).sub(this.grip);
      this.grip.addScaledVector(this.contactGrip, sample.targetBlend);
    }
    ...
    // Supply actors use boat coordinates, so clearance follows the boat's tilt.
    this.position.y += sample.flightArc * sample.flightArcHeight;
