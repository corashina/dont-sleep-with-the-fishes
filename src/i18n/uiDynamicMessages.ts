...
export const uiDynamic = defineMessages({
  ...
  repairItemHelp: { en: (label: string) => `Repair ${label} with Duct Tape.`, pl: (label: string) => `Napraw przedmiot „${label}” taśmą klejącą.`, 'es-AR': (label: string) => `Usá cinta adhesiva para reparar este objeto: ${label}.` },
  ...
  carlitosEnergy: { en: (amount: number, unavailable: boolean) => `CARLITOS: ${amount} ENERGY${unavailable ? ' — UNAVAILABLE' : ''}`, pl: (amount: number, unavailable: boolean) => `CARLITOS: ${amount} ENERGII${unavailable ? ' — NIEDOSTĘPNE' : ''}`, 'es-AR': (amount: number, unavailable: boolean) => `CARLITOS: ${amount} DE ENERGÍA${unavailable ? ' — NO DISPONIBLE' : ''}` },
  ...
  carlitosSpokenEnergy: { en: (amount: number) => `${amount <= 0 ? 'no energy' : `${ENERGY_WORDS[amount] ?? amount} energy`} from Carlitos`, pl: (amount: number) => `${amount} energii Carlitosa`, 'es-AR': (amount: number) => amount <= 0 ? 'Carlitos no tiene energía' : `${SPANISH_ENERGY_WORDS[amount] ?? amount} ${amount === 1 ? 'punto' : 'puntos'} de energía de Carlitos` },
